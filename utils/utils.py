...
def weights_init_xavier2(m):
    classname = m.__class__.__name__
    if classname.find('Conv2d_cd') != -1:
        init.xavier_normal(m.weight.data)
...
def save_train_log(args, save_dir):
    ...
    with open('./OASNet/result/%s/train_log.txt'%save_dir ,'w') as  f:
        now = datetime.now()
        f.write("time:--")
...
def save_model(mean_IOU, best_iou, save_dir, save_prefix, train_loss, test_loss, recall, precision,epoch, net):
    if mean_IOU > best_iou or epoch > 1000-100:
        save_mIoU_dir = './OASNet/result/' + save_dir + '/' + save_prefix + '_best_IoU_IoU.log'
        save_other_metric_dir = './OASNet/result/' + save_dir + '/' + save_prefix + '_best_IoU_other_metric.log'
        now = datetime.now()
        dt_string = now.strftime("%d/%m/%Y %H:%M:%S")
        best_iou = mean_IOU
        ...
        save_ckpt({
            ...
            'loss': test_loss,
            'mean_IOU': mean_IOU,
        }, save_path='./OASNet/result/' + save_dir,
            filename='mIoU_' + '_' + save_prefix + '_'+str(epoch)+'.pth.tar')
...
def make_dir(deep_supervision, dataset, model):
    ...
    os.makedirs('./OASNet/result/%s' % save_dir, exist_ok=True)            # Set save directory
    return save_dir
